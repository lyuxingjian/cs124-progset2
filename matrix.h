...
Matrix<T> Matrix<T>::strassen(const Matrix<T>& Y, unsigned int threshold){
    ...
    auto P1 = A.strassen(F-H, threshold), P2 = (A+B).strassen(H, threshold), P3 = (C+D).strassen(E, threshold), P4 = D.strassen(G-E, threshold), 
            P5 = (A+D).strassen(E+H, threshold), P6 = (B-D).strassen(G+H, threshold), P7 = (C-A).strassen(E+F, threshold);
    // Use in-place operations as much as possible to avoid memory allocation
    ...
    P1 += P5;
    P1 += P7;
    return ((P6).append(P2, 1)).append((P4).append(P1, 1), 0);
}
#endif